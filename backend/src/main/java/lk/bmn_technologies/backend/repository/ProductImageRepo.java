...
public interface ProductImageRepo extends JpaRepository<ProductImageModel, Long>{
    ...
    @Query(value = "SELECT REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(p.image_url, '/upload/', -1), '/', -3), '.png', '') AS publicId FROM product_image_model p WHERE p.product_id = ?1", nativeQuery = true)
