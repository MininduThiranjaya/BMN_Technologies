...
public interface ProjectImageRepo extends JpaRepository<ProjectImageModel, Long>{
    ...
    @Query(value = "SELECT REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(p.image_url, '/upload/', -1), '/', -3), '.png', '') AS publicId FROM project_image_model p WHERE p.project_id = ?1", nativeQuery = true)
